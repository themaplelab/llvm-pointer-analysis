...
void PrintPointsToSet::processPointsToSetForFunc(const Function *func, llvm::DenseMap<size_t, llvm::DenseSet<const llvm::Instruction *>> worklist,
    ...
    while(!bbs.empty()){
        ...
        while(cur){
            printPointsToSet(cur, pts, allocatedPointers);
            if(cur->isTerminator()){
                // outs() << "Terminator: " << *cur << "\n";
                break;
            }
...
PreservedAnalyses PrintPointsToSet::run(Module &m, ModuleAnalysisManager &mam){
    ...
    for(auto &func : m.functions()){
        processPointsToSetForFunc(&func, worklist[&func], pts);
    }
...
void PrintPointsToSet::printPointsToSet(const Instruction *cur, std::map<const Instruction *, std::map<const Instruction *, std::pair<DenseSet<const Value *>, bool>>> pts, std::vector<const AllocaInst*> pointers){
...
/// @param ptr 
/// @return 
DenseSet<const Value *> PrintPointsToSet::trackPointsToSet(const Instruction *cur, const Instruction *ptr, std::map<const Instruction *, std::map<const Instruction *, std::pair<DenseSet<const Value *>, bool>>> pts){
